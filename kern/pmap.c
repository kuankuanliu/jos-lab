...
boot_alloc(uint32_t n)
{
	...
	if (n == 0) {
		...
		if ((uint32_t)nextfree > KERNBASE + npages * PGSIZE) {
			panic("out of memory, boot_alloc should panic.\n");
		} 
		else {
			return result;
		}
...
page_init(void)
{
    ...
    size_t i;
	page_free_list = NULL;
	// set the ref to 1 since the page 0
    pages[0].pp_ref = 1;
	...
	for (i = IOPHYSMEM / PGSIZE; i < npages; i++) {
    if (i < EXTPHYSMEM / PGSIZE) {
	// Comes the IO hole
        pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	// Comes the kernel EXT used in boot_alloc()
    } else if (i < PADDR(boot_alloc(0)) / PGSIZE) {
        pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	// rest of the parts are free.	
    } else {
        pages[i].pp_ref = 0;
        pages[i].pp_link = page_free_list;
        page_free_list = &pages[i];
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(*pde & PTE_P)) { // page table page does NOT exist
		if (!create) {
			return NULL;
		}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    p_pte = pgdir_walk(pgdir, (const void *) va, 1);
    if (!p_pte) {
        panic("boot_map_region cannot allocate memory");
    }
    *p_pte = pa | PTE_P | perm;
    va += PGSIZE;
    pa += PGSIZE;
