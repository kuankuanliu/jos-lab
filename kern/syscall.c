...
sys_exofork(void)
{
    ...
    if ((i = env_alloc(&e, curenv ? curenv->env_id : 0))) {
        return i;
    }
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    // Check perm only includes bits in PTE_SYSCALL
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if (((uintptr_t) srcva & 0xfff) != 0 || ((uintptr_t) dstva & 0xfff) != 0) {
        return -E_INVAL;
    }
    ...
    if ((k = envid2env(srcenvid, &srcenv, 1)) != 0) {
        return k;
    }
    if ((k = envid2env(dstenvid, &dstenv, 1)) != 0) {
        return k;
    }
    ...
    if ((k = page_insert(dstenv->env_pgdir, pp, dstva, perm)) != 0) {
        return k;
    }
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
   ...
   struct Env *e;
    ...
    if ((uint32_t)srcva < UTOP) {
        ...
        if ((k= page_insert(e->env_pgdir, pp, e->env_ipc_dstva, perm)) < 0) {
            return k;
        }
        e->env_ipc_perm = perm;
    } 
